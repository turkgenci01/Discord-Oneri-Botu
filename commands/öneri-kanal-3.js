...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('öneri-kanal-3')
        .setDescription('Üçüncü genel öneri kanalını ayarla')
    ...
    async execute(interaction) {
        ...
        if (guildConfig.suggestionChannels.length >= 3 && guildConfig.suggestionChannels[2]) {
            guildConfig.suggestionChannels[2] = channel.id;
        } else {
            guildConfig.suggestionChannels[2] = channel.id;
        }
        ...
        await interaction.reply({
            content: `✅ ${channel} kanalı **3. Genel Öneri Kanalı** olarak ayarlandı!`,
            ephemeral: true
        });
    }
};
