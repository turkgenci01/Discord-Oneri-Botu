...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('öneri-kanal-4')
        .setDescription('Dördüncü genel öneri kanalını ayarla')
    ...
    async execute(interaction) {
        ...
        if (guildConfig.suggestionChannels.length >= 4 && guildConfig.suggestionChannels[3]) {
            guildConfig.suggestionChannels[3] = channel.id;
        } else {
            guildConfig.suggestionChannels[3] = channel.id;
        }
        ...
        await interaction.reply({
            content: `✅ ${channel} kanalı **4. Genel Öneri Kanalı** olarak ayarlandı!`,
            ephemeral: true
        });
    }
};
